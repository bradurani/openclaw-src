 ...
 * Uses PostgreSQL + pgvector for storage and OpenAI for embeddings.
...
class MemoryDB {
  ...
  async store(entry: {
    ...
    const result = await this.pool.query(
      `INSERT INTO ${this.tableName} (id, text, embedding, importance, category)
       VALUES ($1, $2, $3::vector, $4, $5)
       RETURNING id, text, importance, category, created_at`,
      [id, entry.text, vectorLiteral, entry.importance, entry.category],
    );
  ...
  async search(vector: number[], limit = 5, minScore = 0.5): Promise<MemorySearchResult[]> {
    ...
    return result.rows.map((row) => ({
      entry: {
        id: row.id,
        text: row.text,
        embedding: [], // omit for search results
        importance: row.importance,
        category: row.category,
        created_at: row.created_at,
      },
...
// ============================================================================
// Rule-based capture filter (same heuristics as memory-lancedb)
// ============================================================================

const MEMORY_TRIGGERS = [
  /remember/i,
  /prefer|rather|don't want/i,
  /we decided|we'll use/i,
  /\+\d{10,}/,
  /[\w.-]+@[\w.-]+\.\w+/,
...
function shouldCapture(text: string, opts?: { maxChars?: number }): boolean {
...
const memoryPgvectorPlugin = {
  ...
  description: "PostgreSQL/pgvector-backed long-term memory with auto-recall/capture",
  kind: "memory" as const,
  configSchema: memoryPgvectorConfigSchema,

  register(api: any) {
    const cfg = memoryPgvectorConfigSchema.parse(api.pluginConfig);
    const vectorDim = vectorDimsForModel(cfg.embedding.model);
    const db = new MemoryDB(cfg.connectionString, cfg.tableName, vectorDim);
    const embeddings = new Embeddings(cfg.embedding.apiKey, cfg.embedding.model);

    api.logger.info(
      `memory-pgvector: registered (table: ${cfg.tableName}, model: ${cfg.embedding.model})`,
    );
    ...
    api.registerTool(
      {
        ...
        async execute(
          ...
          const entry = await db.store({ text, embedding: vector, importance, category });
    ...
    api.registerCli(
      ({ program }: { program: any }) => {
        const memory = program.command("pgmem").description("pgvector memory plugin commands");
      ...
      api.on("agent_end", async (event: { success?: boolean; messages?: unknown[] }) => {
        ...
        try {
          ...
          for (const text of toCapture.slice(0, 3)) {
            ...
            await db.store({ text, embedding: vector, importance: 0.7, category });
            stored++;
          }
    ...
    api.registerService({
      ...
      start: () => {
        api.logger.info(
          `memory-pgvector: started (table: ${cfg.tableName}, model: ${cfg.embedding.model})`,
        );
      },
      stop: async () => {
        await db.close();
        api.logger.info("memory-pgvector: stopped, pool closed");
      },
    });
  },
};
