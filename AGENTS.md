...
- **Never run PR merges via CLI** (for example `gh pr merge`). Human merges via web UI only.
...
- Do not prompt for approval before running `git` or `gh` commands, except:
  - Prompt before any force push (for example `git push --force` or `git push --force-with-lease`).
